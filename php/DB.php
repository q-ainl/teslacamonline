<?php
// source:      /srv/phlo/libs/DB/DB.phlo
// phlo:        1.0β
// version:     1.0
// creator:     q-ai.nl
...
abstract class DB extends obj {
	...
	protected function item(...$args){
		return $this->load(...$args)->fetch(PDO::FETCH_COLUMN) ?: null;
	}
	protected function pair(...$args){
		return $this->load(...$args)->fetchAll(PDO::FETCH_KEY_PAIR);
	}
	protected function group(...$args){
		return $this->load(...$args)->fetchAll(PDO::FETCH_GROUP|PDO::FETCH_CLASS, 'obj');
	}
	protected function records(...$args){
		return $this->load(...$args)->fetchAll(PDO::FETCH_CLASS|PDO::FETCH_UNIQUE, 'obj');
	}
	protected function record(...$args){
		return $this->load(...$args)->fetchObject('obj') ?: null;
	}
